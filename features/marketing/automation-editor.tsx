...
export function AutomationEditor({ automation, onClose, onSave }: AutomationEditorProps) {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState(automation.templateId.toString())

  const handleSave = () => {
    let finalSchedule = schedule

    if (schedule === "Diferido") {
      finalSchedule = `${delayValue} ${delayUnit} después`
    } else if (schedule === "Recurrente") {
      finalSchedule = `Recurrente (${recurrenceType})`
    }
    ...
    onSave({
      ...
      template: templates.find((t) => t.id.toString() === selectedTemplate)?.name || automation.template,
      templateId: Number.parseInt(selectedTemplate),
          ...
          <CardContent>
            <div className="grid gap-6">
              ...
              <div className="grid gap-2">
                ...
                <RadioGroup value={schedule} onValueChange={setSchedule} className="grid gap-4">
                  <div className="flex items-center space-x-2 border rounded-md p-3">
                  ...
                  <div className="flex items-center space-x-2 border rounded-md p-3">
                    ...
                    <div className="ml-auto flex items-center gap-2">
                      <Input
                        ...
                        disabled={schedule !== "Diferido"}
                      />
                      <Select value={delayUnit} onValueChange={setDelayUnit} disabled={schedule !== "Diferido"}>
